...
(function() {
    ...
    // --- 1. MODO LEGADO (ADMIN BACKDOOR) ---
    // Adicione ?legacy=true na URL para carregar o login original em caso de emergência
    ...
    // Garante que o layout personalizado só apareça nas rotas de autenticação
    function cleanupLoginInterface() {
        const wrapper = document.getElementById(WRAPPER_ID);
        const style = document.getElementById(STYLE_ID);
        const app = document.getElementById('app');
        
        // Rotas permitidas para o custom login
    ...
    style.id = STYLE_ID;
    style.innerHTML = `
        body, html { height: 100%; margin: 0; padding: 0; font-family: 'Inter', sans-serif; overflow: hidden; }
        ...
        .slide-text h3 { font-size: 1.4rem; font-weight: 600; margin-bottom: 20px; opacity: 0.9; }
        .slide-text p { font-size: 1.1rem; opacity: 0.9; margin-bottom: 30px; line-height: 1.5; }
        .slide-btn { display: inline-block; padding: 12px 30px; border: 2px solid white; color: white; border-radius: 50px; text-decoration: none; font-weight: 700; transition: 0.3s; }
        .slide-btn:hover { background: white; color: ${CONFIG.colors.primary}; }
        
        .slide-img { width: 45%; display: flex; justify-content: center; z-index: 1; }
        /* Ajuste para imagens do Unsplash preencherem bem */
        .slide-img img { 
            width: 100%; max-width: 400px; height: 300px; 
            object-fit: cover; border-radius: 20px;
            box-shadow: 0 20px 40px rgba(0,0,0,0.3);
        }
        .small-logo { height: 40px; margin-bottom: 25px; display: block; filter: brightness(0) invert(1); } 
        
        .nav-btn { position: absolute; top: 50%; transform: translateY(-50%); background: rgba(255,255,255,0.1); border: none; cursor: pointer; width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; transition: 0.3s; z-index: 10; }
        ...
        .dot { width: 12px; height: 12px; border-radius: 50%; background: ${CONFIG.colors.dotColor}; cursor: pointer; transition: 0.3s; }
        .dot.active { background: ${CONFIG.colors.dotActive}; width: 30px; border-radius: 15px; }

        /* --- LOGIN CARD (DIREITA) --- */
        .login-card-wrapper { 
            width: 450px; height: 100%; background: white; 
            padding: 0 50px; border-radius: 24px; 
            box-shadow: 0 30px 60px -12px rgba(0, 0, 0, 0.5); z-index: 20;
        ...
        form input:not([maxlength="1"]) {
            ...
            border: 1px solid #e5e7eb !important; border-radius: 50px !important;
            padding: 16px 20px 16px 55px !important; width: 100% !important; box-sizing: border-box !important;
            margin-bottom: 20px !important; background-repeat: no-repeat !important;
            background-position: 20px center !important; font-size: 1rem !important; font-weight: 500 !important;
        }
        ...
        /* Botão Entrar */
        form button[type="submit"] { background-color: ${CONFIG.colors.primary} !important; border-radius: 50px !important; height: 50px !important; width: 100% !important; color: white !important; font-weight: bold !important; border: none !important; cursor: pointer; box-shadow: 0 10px 20px -5px rgba(0,0,0,0.2); transition: 0.2s; margin-top: 10px; }
        ...
        #form-container h2 { font-size: 1.5rem !important; font-weight: 700 !important; color: #111827 !important; margin-bottom: 10px !important; text-align: center; }
        ...
        #form-container a.button, #form-container button.button { border-radius: 50px !important; }

        /* --- FOOTER DESKTOP --- */
        .custom-footer { position: fixed; bottom: 0; left: 0; width: 100%; background: white; height: 50px; display: flex; align-items: center; justify-content: center; gap: 40px; box-shadow: 0 -5px 20px rgba(0,0,0,0.05); z-index: 100; }
        .custom-footer a { color: #64748b; text-decoration: none; font-size: 13px; font-weight: 500; }
    ...
    // Detecta Login, 2FA e SSO dinamicamente
    const checkForm = setInterval(() => {
        ...
        if (newForm && !myContainer.contains(newForm)) {
             // Tenta pegar o container do formulário para trazer botões SSO/Títulos junto
